...
export function UserManagementPanel({
  ...
  const [newUserRole, setNewUserRole] = useState<"user" | "manager" | "root">(
    "user",
  );
  ...
  const handleToggleUserGroup = async (
    userId: string,
    groupId: string,
  ...
  const handleToggleGroupMember = async (
    groupId: string,
    userId: string,
    isCurrentlyMember: boolean,
  ) => {
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-3 space-y-2">
                {safeUsers.map((user) => {
                  const isMember =
                    editingGroup.memberIds?.includes(user.id) || false;
                  return (
                    <label
                      key={user.id}
          ...
          <div className="space-y-4">
              ...
              <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                ...
                <ul className="space-y-1">
                  {passwordRules.map((rule) => {
                    const isValid = newUserPassword
                      ? rule.test(newUserPassword)
                      : false;
                    return (
                      <li
                        key={rule.id}
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button
                ...
                disabled={
                  !passwordRules.every(
                    (r) => newUserPassword && r.test(newUserPassword),
                  )
                }
              >
                Create
